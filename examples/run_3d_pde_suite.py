...
def discover(name, data, terms, max_terms=4):
    ...
    result = RNSPDEDiscoverer(max_terms=max_terms, top_k=min(10, len(terms)), cem_iterations=3, population=25, random_state=5).fit(system)
    print(f"\n{name}")
...
def main() -> None:
    ...
    cases = [
        ("3D heat", make_3d_heat(), scalar_terms, 3),
        ("3D Burgers", make_3d_burgers(), scalar_terms, 4),
        ("3D Allen-Cahn", make_3d_allen_cahn(), scalar_terms, 5),
        ("3D Cahn-Hilliard", make_3d_cahn_hilliard(), scalar_terms, 6),
        ("3D KdV", make_3d_kdv(), scalar_terms, 3),
        ("3D reaction-diffusion", make_3d_reaction_diffusion(), scalar_terms, 5),
        ("3D reaction-advection-diffusion", make_3d_reaction_advection_diffusion(nx=14, ny=12, nz=10, nt=9), scalar_terms, 7),
    ]
    for case in cases:
        discover(*case)

    nls = make_3d_nls()
    discover("3D NLS / GP (real component)", nls, build_nls_library("a"), 4)

    ns = make_3d_ns()
    discover("3D Navier-Stokes / ABC flow (u component)", ns, build_ns_library("u"), 7)
