...
def make_3d_reaction_advection_diffusion(nx: int = 18, ny: int = 16, nz: int = 14, nt: int = 11, noise: float = 0.0, seed: int = 7, coeffs: dict[str, float] | None = None) -> FieldData:
    params = {"u": -0.15, "u_x": -0.80, "u_y": 0.45, "u_z": -0.30, "u_xx": 0.06, "u_yy": 0.06, "u_zz": 0.06}
    if coeffs:
        params.update(coeffs)
    cx, cy, cz = -params["u_x"], -params["u_y"], -params["u_z"]
    alpha, nu = params["u"], params["u_xx"]
...
def make_3d_heat(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9, nu: float = 0.08) -> FieldData:
    modes = [(1.0, 1, 1, 1, 0.1), (0.4, 2, 1, 2, -0.3)]
    def formula(x, y, z, t):
        return sum(a * math.exp(-nu * (kx*kx+ky*ky+kz*kz) * t) * math.sin(kx*x + ky*y + kz*z + ph) for a, kx, ky, kz, ph in modes)
    return _field_from_formula(nx, ny, nz, nt, formula, {"u_xx": nu, "u_yy": nu, "u_zz": nu}, t_end=0.5)


def make_3d_burgers(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9) -> FieldData:
    # Manufactured 3-D scalar Burgers-like data in expanded columns.
    return _manufactured_scalar(nx, ny, nz, nt, {"u_u_x": -1.0, "u_xx": 0.04, "u_yy": 0.04, "u_zz": 0.04}, amplitude=0.28)


def make_3d_allen_cahn(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9) -> FieldData:
    return _manufactured_scalar(nx, ny, nz, nt, {"u": 1.0, "u_cubed": -1.0, "u_xx": 0.03, "u_yy": 0.03, "u_zz": 0.03}, amplitude=0.35)


def make_3d_cahn_hilliard(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9) -> FieldData:
    return _manufactured_scalar(nx, ny, nz, nt, {"u_xx": -0.15, "u_yy": -0.15, "u_zz": -0.15, "u_xxxx": -0.01, "u_yyyy": -0.01, "u_zzzz": -0.01}, amplitude=0.4)


def make_3d_kdv(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9) -> FieldData:
    return _manufactured_scalar(nx, ny, nz, nt, {"u_u_x": -6.0, "u_xxx": -0.08}, amplitude=0.12)


def make_3d_reaction_diffusion(nx: int = 14, ny: int = 12, nz: int = 10, nt: int = 9) -> FieldData:
    return _manufactured_scalar(nx, ny, nz, nt, {"u": 0.7, "u_cubed": -0.9, "u_xx": 0.05, "u_yy": 0.05, "u_zz": 0.05}, amplitude=0.32)
...
def make_3d_nls(nx: int = 12, ny: int = 10, nz: int = 8, nt: int = 9) -> FieldData:
    ...
    return FieldData(a, shape, x, y, z, t, {"b_xx": -1.0, "b_yy": -1.0, "b_zz": -1.0, "abspsi_squared_b": -g}, {"a": a, "b": b}, "a")
...
def make_3d_ns(nx: int = 12, ny: int = 10, nz: int = 8, nt: int = 9, nu: float = 0.04) -> FieldData:
    ...
    return FieldData(u, shape, x, y, z, t, {"u_xx": nu, "u_yy": nu, "u_zz": nu}, {"u": u, "v": v, "w": w, "p": p}, "u")
