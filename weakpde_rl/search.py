...
class RNSPDEDiscoverer:
    ...
    def _complete_expanded_support(self, system: WeakSystem, support: tuple[int, ...]) -> tuple[int, ...]:
        """Complete compact derivative families as fully expanded terms."""
        names = system.names
        selected = set(support)
        groups = [
            ("u_xx", "u_yy", "u_zz"),
            ("u_xxxx", "u_yyyy", "u_zzzz"),
            ("a_xx", "a_yy", "a_zz"),
            ("b_xx", "b_yy", "b_zz"),
        ]
        groups.extend((f"{component}_xx", f"{component}_yy", f"{component}_zz") for component in ("u", "v", "w"))
        norms = column_norms(system.raw_theta)
        for group in groups:
            indexes = [names.index(name) for name in group if name in names and norms[names.index(name)] > 1e-10]
            if indexes and any(index in selected for index in indexes):
    ...
    def fit(self, system: WeakSystem) -> DiscoveryResult:
        ...
        best_support = self._complete_expanded_support(system, max(beam, key=lambda s: self._score(system, s)[0]))
