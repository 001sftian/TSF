...
2. **Fourth-order derivatives**: `u_t`, `u_x`, `u_xx`, `u_yy`, `u_zz`, `u_xxx` and fourth-order
   expanded derivative terms are estimated with fourth-order finite-difference
...
5. **Adaptive libraries**: scalar atoms, individual derivative columns,
   nonlinear advection/reaction terms, NLS split-complex terms, and NS
   vector-pressure terms. Compact operators such as Laplacian or biharmonic are
   not library columns; equations are discovered in fully expanded form.
...
- 3-D Cahn-Hilliard-style fully expanded fourth-order equation
...
```text
u_t = -0.15*u -0.80*u_x +0.45*u_y -0.30*u_z +0.06*u_xx +0.06*u_yy +0.06*u_zz
```
